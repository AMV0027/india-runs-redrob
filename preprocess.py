...
def run_preprocessing(candidates_path, output_dir):
    ...
    selected_pairs = prioritized_candidates[:3000]
    valid_candidates = [pair[0] for pair in selected_pairs]
    print(f"Selected top {len(valid_candidates)} candidates for embedding.")
    ...
    print("Embedding Headline/Summary...")
    embeds_hs = model.encode(headline_summaries, show_progress_bar=True, batch_size=128)
    
    print("Embedding Current Roles...")
    embeds_cr = model.encode(current_roles, show_progress_bar=True, batch_size=128)
    
    print("Embedding Past Roles...")
    embeds_pr = model.encode(past_roles, show_progress_bar=True, batch_size=128)
